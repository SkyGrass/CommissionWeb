  ...
  <div>
    ...
    >
      ...
      >
        ...
        <Row :gutter="16">
          <Col span="12">
            <FormItem label="用户部门" prop="userDept" v-if="false">
              <Input
                v-model="formModel.fields.userDept"
                placeholder="请输入用户部门"
              />
            </FormItem>
    ...
    <Drawer
      :title="formTitle"
      v-model="formModel.binded"
      width="600"
      :mask-closable="false"
      :mask="true"
      :styles="styles"
      @on-visible-change="initUserBind"
    >
      <Form
        :model="formModel.bindfields"
        ref="formUserBind"
        :rules="formModel.bindrules"
        label-position="top"
      >
        <Row :gutter="16">
          <Col span="12">
            <FormItem label="业务员档案" prop="salesmanId">
              <Select v-model="formModel.bindfields.salesmanId">
                <Option
...
import { getSalesManSelectList } from "@/api/base/saleman";
export default {
  name: "rbac_user_page",
  components: {
    DzTable,
      ...
      stores: {
        user: {
          ...
          columns: [
            ...
            // { title: "部门", key: "userDept", width: 250 },
            {
              title: "用户类型",
              key: "userType",
              slot: "userType",
  ...
  computed: {
    ...
    selectedRowsId() {
      return this.formModel.selection.map((x) => x.fGuid);
    },
  },
  methods: {
    loadUserList() {
    ...
    initUserBind(state) {
      if (state) {
        this.loadSalesmansList();
      }
    },
    handleOpenFormWindow() {
      this.formModel.opened = true;
    },
    handleOpenBindFormWindow() {
      this.formModel.binded = true;
    },
    ...
    handleSwitchFormModeToBind() {
      this.formModel.mode = "bind";
      this.handleOpenBindFormWindow();
    },
    ...
    handleBind(row) {
      this.handleSwitchFormModeToBind();
      this.handleResetFormUserBind();
      this.doLoadUserBind(row.fGuid);
    },
    handleEdit(row) {
      this.handleSwitchFormModeToEdit();
      this.handleResetFormUser();
      this.doLoadUser(row.fGuid);
    },
    ...
    handleBindUser() {
      let valid = this.validateUserForm("formUserBind");
      if (valid) {
        if (this.formModel.mode === "bind") {
          this.doBindUser();
        }
    ...
    handleResetFormUserBind() {
      this.$refs["formUserBind"].resetFields();
    },
    doCreateUser() {
    ...
    validateUserForm(formname) {
      let _valid = false;
      this.$refs[formname || "formUser"].validate((valid) => {
        if (!valid) {
    ...
    doLoadUserBind(guid) {
      getBindUser({ guid }).then(({ data: { state, data } }) => {
        if (data) {
          const { id, salesmanId } = data;
          this.formModel.bindfields.id = id;
          this.formModel.bindfields.salesmanId = salesmanId;
        } else {
          this.formModel.bindfields.id = -1;
        }
        this.formModel.bindfields.userId = guid;
      });
    },
    handleDelete(row) {
      this.doDelete(row.fGuid);
    },
    doDelete(ids) {
      if (!ids) {
    ...
    handleAssignRole(row) {
      ...
      this.formAssignRole.userGuid = row.fGuid;
      this.loadUserRoleList(row.fGuid);
    },
    handleSaveUserRoles() {
      var data = {
